...
class ServerActivity : BaseActivity() {
    ...
    private val container_allow_insecure: LinearLayout? by lazy {
        findViewById(R.id.lay_allow_insecure)
    }
    ...
    private val sp_stream_fingerprint: Spinner? by lazy {
        findViewById(R.id.sp_stream_fingerprint)
    } // uTLS
    private val container_fingerprint: LinearLayout? by lazy {
        findViewById(R.id.lay_stream_fingerprint)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sp_network?.onItemSelectedListener =
                            ...
                            getString(
                                when (networks[position]) {
                                    ...
                                    NetworkType.HTTP_UPGRADE.type ->
                                        R.string.server_lab_request_host_httpupgrade
                        ...
                        Utils.getEditable(
                            getString(
                                when (networks[position]) {
                                    ...
                                    NetworkType.HTTP_UPGRADE.type ->
                                        R.string.server_lab_path_httpupgrade
        ...
        sp_stream_security?.onItemSelectedListener =
            object : AdapterView.OnItemSelectedListener {
                override fun onItemSelected(
                    parent: AdapterView<*>?,
                    view: View?,
                    ...
                    when {
                        ...
                        // Case 2: TLS value
                        isTLS -> {
                            listOf(container_sni, container_fingerprint, container_alpn).forEach {
                                it?.visibility = View.VISIBLE
                            }
                            container_allow_insecure?.visibility = View.VISIBLE
                            listOf(container_public_key, container_short_id, container_spider_x)
                                .forEach { it?.visibility = View.GONE }
                        }
    ...
    private fun saveServer(): Boolean {
        ...
        sp_stream_security?.let {
            if (
                config.configType == EConfigType.TROJAN &&
                    TextUtils.isEmpty(streamSecuritys[it.selectedItemPosition])
            ) {
    ...
    private fun saveCommon(config: ProfileItem) {
        ...
        } else if (config.configType == EConfigType.TROJAN) {} else if (
